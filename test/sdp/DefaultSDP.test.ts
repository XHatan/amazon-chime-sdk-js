...
describe('DefaultSDP', () => {
  ...
  describe('withOldFashionedMungingSimulcast', () => {
    ...
    it('returns same SDP if SDP has no video section', () => {
      const simulcastedSDP = new DefaultSDP(SDPMock.VIDEO_HOST_AUDIO_ANSWER).withOldFashionedMungingSimulcast(2);
    ...
    it('returns same SDP if original SDP only has recv video section', () => {
      const simulcastedSDP = new DefaultSDP(SDPMock.LOCAL_OFFER_WITH_RECV_VIDEO).withOldFashionedMungingSimulcast(
        2
      );
